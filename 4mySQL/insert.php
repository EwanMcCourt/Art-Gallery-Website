...
echo "<p>$name $email $phone $twitter $facebook</p>";

if ($name==="") die("Sorry a name is needed");
if ( ($phone==="") && ($email==="") && ($twitter==="") && ($facebook==="")) die("Sorry some contact details needed");

$sql = "INSERT INTO `000_phonebook` (`id`, `name`, `email`, `phone`, `twitter`, `facebook`)".
    " VALUES (NULL, '$name', '$phone', '$email', '$twitter', '$facebook')";

//echo "<p>$sql</p>";

if ($conn->query($sql)){
    echo "<p>Insert successfull with id ".$conn->insert_id."</p>";

    $sql = "SELECT * FROM `000_phonebook`  WHERE `id` = ".$conn->insert_id;
